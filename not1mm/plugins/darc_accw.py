...
#   DARC Ausbildungs Contest CW (trainig contest CW)
#   Status:	Active
...
#   Mode:	CW
#   Bands:	80m
#   Classes:	Single Op, single TRX, max. 100W
...
#   Multipliers:	DOK (DARC/VFDB), 'NM' does not count + each WPX-prefix
#   Score Calculation:	Total score = total QSO points x total mults
#   Mail logs to:	(none)
#   Find rules at:	https://www.darc.de/der-club/referate/conteste/cwa/darc-cwa-ausschreibung/
...
def prefill(self):
    ...
    if self.other_2.text() == "":
        ...
        query = f"select NR from dxlog where Call = '{call}' and ContestName = 'ACCW' order by ts desc;"
        logger.debug(query)
        result = self.database.exec_sql(query)
...
def adif(self):
    ...
    gen_adif(self, cabrillo_name, "ACCW")
